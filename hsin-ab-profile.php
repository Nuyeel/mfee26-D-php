...
// $good = $pdo->query("SELECT * FROM `good_deed_test_record` JOIN `member` ON  `good_deed_test_record`.`member_sid`= `member`.`sid` ORDER BY `member`.`sid` ASC WHERE `good_deed_test_record`.`member_sid`= '$sid'")->fetch(PDO::FETCH_NUM)[0];

// $sql = sprintf(
//     "SELECT * FROM `good_deed_test_record` JOIN `member` ON  `good_deed_test_record`.`member_sid`= `member`.`sid` ORDER BY `member`.`sid` ");

// if (isset()) {    
// $good = $pdo->query($sql)->fetchAll();
// }
