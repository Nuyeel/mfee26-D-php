<?php
require __DIR__ . "./test_connect_db.php";
/*
$sql = "INSERT INTO `place2` (
    `year`, `month`, `city`, `dist`, `quota`, `booked`
    ) VALUES (
        '2035', '10', '新北市', '板橋區', '3', '1'
    );";
$stmt = $pdo->query($sql);
...
for ($i = 0; $i < 100; $i++) {
    $key_city = array_keys($cityDist);
    shuffle($key_city);
    $city = $key_city[0];
    $distAr = $cityDist[$city];
    shuffle($distAr);
    $dist = $distAr[0];

    $ts = rand(strtotime('2022-08-24'), strtotime('2122-08-24'));
    $quota = floor(rand(0, 5));
    $booked = floor(rand(0, $quota));
