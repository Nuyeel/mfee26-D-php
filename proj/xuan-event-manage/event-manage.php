...
// 計算總共有幾筆，以建立頁籤 
$t_sql = "SELECT COUNT(*) FROM( (`npo_act` JOIN `npo_act_type` ON `npo_act`.`type_sid` = `npo_act_type`.`typesid`) INNER JOIN `npo_name` ON `npo_act`.`npo_name` = `npo_name`.`npo_name`) INNER JOIN `city_type` ON `npo_act`.`place_city`= `city_type`.`city_sid` ";
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                <a class="page-link" href="?page=<?= $page - 1 ?>"><i class="fa-solid fa-angles-left"></i></a>
            </li>
        ...
        <tbody>
            <?php foreach ($rows as $r) : ?>
                <tr>
                    ...
                    <td>
                        <a href="event-edit.php?sid=<?= $r['sid'] ?>">
                    ...
                    <td>
                        ...
                        <!-- <a href="# " onclick="trashCanClicked(event); return false">
...
<script>
    function delete_it(sid) {
        if (confirm(`確定要刪除編號為 ${sid} 的資料嗎?`)) {
            location.href = `event-delete.php?sid=${sid}`;
        }
    }
